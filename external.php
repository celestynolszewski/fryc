<?php
  ...
  function logwrite($string,$handler) {
      echo $string.PHP_EOL;
      if($handler) {
        fwrite($handler,$string.PHP_EOL);     
      }
  }

  // domena -> mail server. mozna to zrobic przez odpytywanie dns o rekord mx ale dodaloby jedynie dodatkowe opoznienie
  // przeoranie pliku bez ladowania do tablicy wydajnosciowo tez nie jest wcale takie zle bo ponizej 0.0001 sekundy
  function getHost ($email) {

      $split = explode("@",$email);
      $domain = $split[1];
            
    // przetwarzanie pliku servers.txt
    (file_exists("servers.txt") && filesize("servers.txt")>0 ? $file_servers = fopen("servers.txt","r") : $file_servers = false);

     if($file_servers) {
        $servers = array();
        while(($s_line = fgets($file_servers)) !== FALSE) {
             $servers = explode(";",$s_line);
             
             if(array_search($domain, $servers)) $imap=$servers[0]; 
             
        }
      } else die('Brak pliku servers.txt! Bez niego nie podzialam :(');
  
      if($file_servers) fclose($file_servers);
